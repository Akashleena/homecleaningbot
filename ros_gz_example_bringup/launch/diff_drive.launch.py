...
def generate_launch_description():
    ...
    # Load URDF for robot_state_publisher (TF tree, Nav2, MoveIt)
    urdf_file = os.path.join(pkg_project_description, 'urdf', 'robot.urdf')
    with open(urdf_file, 'r') as infp:
        robot_desc = infp.read()

    # Load SDF for Gazebo spawning (physics, sensors, plugins)
    ...
    # Publishes TF tree from URDF - needed by Nav2, SLAM, MoveIt
    ...
    # Spawn robot in Gazebo using SDF (has all sensors and plugins)
    spawn_robot = Node(
        package='ros_gz_sim',
        executable='create',
        arguments=[
